...
pipeline {
    ...
    environment {
        ...
        AWS_ECS_CPU = '256'
        AWS_ECS_MEMORY = '512'
        AWS_ECS_CLUSTER = 'DevOps-Test-ECS-Nginx'
    ...
    stages {
        ...
        stage('Build Docker Image') {
            steps {
                withCredentials([string(credentialsId: 'AWS_SECRET', variable: 'AWS_ECR_URL')]) {
                    script {
        ...
        stage('Deploy in ECS') {
            steps {
                withCredentials([string(credentialsId: 'AWS_SECRET', variable: 'AWS_ECS_EXECUTION_ROL'),string(credentialsId: 'AWS_SECRET', variable: 'AWS_ECR_URL')]) {
                    script {
